...
/// Type-erased view of a PendingMap, for operations spanning every map.
protocol AnyPendingMap {
  func failAll(error: Error)
  func clearAll()
}

extension PendingMap: AnyPendingMap {}
...
final class PendingOperations {
  ...
  private var all: [AnyPendingMap] {
    [connect, disconnect, discover, rssi, charRead, charWrite, setNotify, descRead, descWrite]
  }
  ...
  func failAll(error: Error) {
    all.forEach { $0.failAll(error: error) }
  }
