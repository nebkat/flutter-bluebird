...
object Proto {
    ...
    fun charOpFields(characteristic: BluetoothGattCharacteristic): List<Any> = listOf(
        uuid128(characteristic.service.uuid),
        characteristic.service.instanceId,
        uuid128(characteristic.uuid),
        characteristic.instanceId,
    )

    fun readCharKey(address: String, c: BluetoothGattCharacteristic) = OpKey.ReadChar(
        address, uuid128(c.service.uuid), c.service.instanceId, uuid128(c.uuid), c.instanceId)

    fun writeCharKey(address: String, c: BluetoothGattCharacteristic) = OpKey.WriteChar(
        address, uuid128(c.service.uuid), c.service.instanceId, uuid128(c.uuid), c.instanceId)

    fun setNotifyKey(address: String, c: BluetoothGattCharacteristic) = OpKey.SetNotify(
        address, uuid128(c.service.uuid), c.service.instanceId, uuid128(c.uuid), c.instanceId)

    fun readDescKey(address: String, d: BluetoothGattDescriptor): OpKey.ReadDesc {
        val c = d.characteristic
        return OpKey.ReadDesc(
            address, uuid128(c.service.uuid), c.service.instanceId,
            uuid128(c.uuid), c.instanceId, uuid128(d.uuid))
    }

    fun writeDescKey(address: String, d: BluetoothGattDescriptor): OpKey.WriteDesc {
        val c = d.characteristic
        return OpKey.WriteDesc(
            address, uuid128(c.service.uuid), c.service.instanceId,
            uuid128(c.uuid), c.instanceId, uuid128(d.uuid))
    }
