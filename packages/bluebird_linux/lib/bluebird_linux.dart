...
extension on BlueZGattCharacteristic {
  int instanceId(BlueZGattService service) {
    ...
    for (final c in service.characteristics) {
      if (Uuid.fromBytes(c.uuid.value) == target) {
  ...
  Uuid get uuid2 => Uuid.fromBytes(uuid.value);
}
...
final class BluebirdLinux extends BluebirdPlatform {
                  ...
                  characteristic.propertiesChanged.where(
                    ...
                    (properties) {
                      return BmCharacteristicData(
                        ...
                        serviceUuid: service.uuid2,
                        characteristicUuid: characteristic.uuid2,
          ...
          advertisements: [
            BmScanAdvertisement(
              ...
              serviceData: device.serviceData.map(
                (uuid, value) {
                  return MapEntry(Uuid.fromBytes(uuid.value), value);
                },
              ),
              serviceUuids: device.uuids.map(
                (uuid) {
                  return Uuid.fromBytes(uuid.value);
                },
              ).toList(),
              rssi: device.rssi,
            ),
                ...
                characteristics: service.characteristics.map(
                  (characteristic) {
                    return BmBluetoothCharacteristic(
                      ...
                      characteristicUuid: Uuid.fromBytes(
  ...
  Future<bool> writeDescriptor(
    ...
    try {
      ...
      final device = _client.devices.singleWhere(
        (device) => device.address == request.address,
      );
  ...
  static void registerWith() {
    BluebirdPlatform.instance = BluebirdLinux();
  }

  Future<void> _initBluebird() async {
    if (_initialized) {
      return;
    }
      ...
      (devices) {
        if (_logLevel == LogLevel.verbose) {
          BluebirdPlatform.log(
            '[Bluebird-Linux] devices changed ${devices.map((device) => device.address).toList()}',
          );
        }
            ...
            (device) {
              return device.propertiesChanged.switchMap(
                (properties) {
                  if (_logLevel == LogLevel.verbose) {
                    BluebirdPlatform.log(
                      '[Bluebird-Linux] device ${device.address} properties changed $properties',
                    );
                  }
                      ...
                      streams.add(
                        characteristic.propertiesChanged.map(
                          (properties) {
                            if (_logLevel == LogLevel.verbose) {
                              BluebirdPlatform.log(
                                '[Bluebird-Linux] device ${device.remoteId} service ${service.uuid} characteristic ${characteristic.uuid} properties changed $properties',
                              );
                            }
                          },
                        ),
