...
class Bluebird {
  ...
  static Stream<BluetoothAdapterState> get adapterState async* {
    ...
    yield* Bluebird.extractEventStream<OnAdapterStateChangedEvent>()
  ...
  static Future<void> startScan({
    ...
    await Mutex.scan.protect(() async {
      ...
      Stream<OnScanResponseEvent> responseStream = Bluebird.extractEventStream<OnScanResponseEvent>();
      ...
      _scanSubscriptions.add(scanBuffer.listen((OnScanResponseEvent response) {
        ...
        // push entire list
        if (!oneByOne) {
          _scanResults.add(output.values.toList());
        }
      }));
      ...
      if (removeIfGone != null) {
        _scanSubscriptions.add(Stream.periodic(Duration(milliseconds: 250)).listen((_) {
          ...
          _scanResults.add(output.values.toList()); // push to stream
        }));
      }
  ...
  static Future<void> stopScan() async {
    await Mutex.scan.protect(() async {
      if (isScanningNow) {
        ...
        BluebirdPlatform.log("[Bluebird] stopScan: already stopped");
      }
    });
  }
  ...
  static void cancelWhenScanComplete(StreamSubscription subscription) {
    Bluebird._scanSubscriptions.add(subscription);
  }
    ...
    switch (event) {
      ...
      case BmScanFailedEvent():
        _scanResults.addError(
          BluebirdException("scan", BluebirdErrorCode.platform, "(${event.errorCode}) ${event.errorString}"),
        );
        _stopScan(invokePlatform: false);
...
class BluebirdException implements Exception {
  ...
  BluebirdException(this.function, this.code, [this.description]);

  @override
  String toString() => 'BluebirdException | $function | bluebird-code: $code | $description';
}
